...
2.10. **Customer satisfaction (CSAT)** — When a ticket is closed, the ticket owner is prompted to rate the support experience on a 1–5 star scale. The user may optionally add a text comment along with the rating. Submitting a rating creates a special CSAT post on the ticket that does **not** re-open it. The assigned agent (or all agents if none was assigned) receives a notification about the rating and comment. The rating is shown on the ticket detail page. A user can only rate once per ticket closure; if the ticket is re-opened and closed again, the user can submit a new rating that replaces the previous one.
