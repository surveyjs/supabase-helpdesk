...
- Removing a duplicate link automatically sets the ticket to **open**.
- Escalating a ticket sets its status to **open** (if not already).
- Closing a ticket automatically removes the assigned agent.
...
| **Medium** | Yellow | Moderate impact, workaround available. Non-critical functionality affected. This is the default severity. |
...
3.2. **View my tickets** — The home page shows a list of the current user's tickets sorted by last-updated. Each entry shows the title, last-updated date, and a color-coded status badge (green = open, yellow = pending, gray = closed). Clicking a ticket opens the detail page.
...
8.2. **View all tickets** — The dashboard shows ALL tickets in the system (both private and public), with the submitter's email, last-updated date, post count, and status badge.
...
8.6. **Result count** — The dashboard shows "N ticket(s) found" above the list.

8.7. **All filters are URL-based** — Filters use URL search params so the page is bookmarkable and shareable.

#### 9. Agent Actions on Tickets

9.1. **Change status** — On a ticket detail page, an agent sees status action buttons depending on the current state: "Mark Pending" (sets to pending), "Close Ticket" (sets to closed), and "Re-open Ticket" / "Mark Open" (sets to open). The available buttons adapt to the ticket's current status so the agent can transition between any of the three states. Regular users do not see these buttons. Closing a ticket automatically removes the assigned agent. Status transition rules are defined in section 2.
...
14.2. **Email notifications for agents** — Agents receive email notifications when: a new ticket is created, a user replies to a ticket assigned to them, a ticket is assigned to them, a user replies to a closed ticket (auto-transitions to open), a ticket is escalated to them, or a user marks their post as solved / submits or changes a CSAT rating on their solved post. An agent does not receive notifications for their own actions.
...
15.4. **Unknown sender** — If an incoming email is from an address that does not match any registered user, the system still creates a ticket: it auto-registers a new user account using the sender's email address and creates the ticket on their behalf. The ticket is always **private** (regardless of the admin-configured default). All other defaults are the same as for known users (system default ticket type, Medium severity, no category, no tags). The system sends an email to the new user informing them that an account has been created and providing a link to set their password. The email template for this notification is configurable by the admin with a "Reset to default" option.
