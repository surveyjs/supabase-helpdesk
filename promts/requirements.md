...
- **Left side**: App name "HelpDesk" (links to home), "My Tickets" link, (if KB enabled — see 19.5) "Help Center" link, (for agents) "Agent Dashboard" link, (for agents) "Manage Articles" link, (for agents) "Canned Responses" link, (for admins only) "Reports" link, and (for admins only) "Setup" link.
