...
3.13. **Custom fields** — Tickets can have additional custom fields defined by the admin (see 16.14). Custom fields are stored as a JSON object on the ticket. On the ticket creation form, custom fields are displayed after the standard fields. On the ticket detail page, custom fields are rendered as SurveyJS questions inside the editable sidebar, using the naming convention `custom_fields.<custom_field_name>` so each question maps 1:1 to a Supabase custom-field entry. By default, the two ticket-detail SurveyJS templates (`survey_ticket_detail_agent_template`, `survey_ticket_detail_user_template`) have `autoGenerateCustomFields: true`, which causes the application to append auto-generated `custom_fields.<name>` questions (one per defined custom field) to the bottom of the sidebar. An admin may disable this toggle and author the custom-field questions manually in the template JSON, in which case every `custom_fields.<name>` question must reference an existing custom field. Custom-field questions are visible to all viewers of the ticket but are editable only by the ticket owner and agents; they are read-only when the ticket is merged or the viewer is blocked. Edits autosave via the standard ticket-detail dispatcher.
...
**Environment variable fallback** — As a convenience for initial setup or containerised deployments, AI connection settings can be pre-configured via environment variables: `AI_PROVIDER`, `AI_API_KEY`, `AI_MODEL`, and `AI_CUSTOM_ENDPOINT_URL`. When the Vault contains no API key, the system falls back to `AI_API_KEY`. For provider, model, and endpoint URL, values stored in the database always take precedence over environment variables; the env vars are used only when the corresponding database field is empty. The admin page shows the active key source: "Stored in Vault" when a key has been saved through the UI, or "Loaded from AI_API_KEY environment variable" when the fallback is active. An admin can override the environment key at any time by entering a new key in the API Key field and saving — this stores the key in Vault and takes precedence from that point on.
