...
14.2. **Email notifications for agents** — Agents receive email notifications when: a user replies to a ticket assigned to them, a ticket is assigned to them (with reason, if provided), a user replies to a closed ticket (auto-transitions to open), a user submits or changes a CSAT rating on a ticket assigned to them, an SLA target on a ticket assigned to them is approaching the threshold, an SLA target on a ticket assigned to them is breached, or another agent adds a note to a ticket assigned to them. Admins additionally receive SLA breach notifications for all tickets, not just those assigned to them (see 17.5). An agent does not receive notifications for their own actions (e.g., if an admin closes a ticket assigned to themselves, they do not receive the closure notification). Note: new ticket creation does **not** trigger a notification to all agents — agents see new unassigned tickets via the Agent Dashboard.
