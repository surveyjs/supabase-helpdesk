...
function SeverityMappingSection({ policies, mappings }: { policies: SlaPolicy[]; mappings: Record<string, string | null> }) {
  return (
    <section className="mb-8">
...
function BusinessHoursSection({ businessHours }: { businessHours: { timezone: string; schedule: Record<string, DaySchedule> } }) {
  return (
    <section className="mb-8">
      ...
      <form action={updateBusinessHours} data-testid="business-hours-form">
        <div className="mb-4">
          ...
          <select name="timezone" defaultValue={businessHours.timezone}
            className="rounded border border-gray-300 px-2 py-1.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none"
...
export function SlaSettingsForm({
  ...
  return (
    <div className="max-w-3xl">
      <PolicySection policies={initialPolicies} />
      <SeverityMappingSection policies={initialPolicies} mappings={initialMappings} />
      <BusinessHoursSection businessHours={initialBH} />
      <ThresholdSection threshold={initialThreshold} />
    </div>
  );
}
